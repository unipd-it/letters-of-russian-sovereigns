...
final class RecipientFilterParameter implements FilterParameterInterface, ExpressionBuilderInterface
{
    ...
    {
        return [
            'label' => 'controller.letter.list.filter.recipient',
            'attr' => [
                'class' => '',
    ...
    public function buildWhereExpression(QueryBuilder $queryBuilder, $formData, string $entityAlias): ?string
    {
        ...
        return (string) $queryBuilder->expr()->in($entityAlias.'.recipient', $ids);
    }

    private function createQueryBuilder(): callable
    {
        return function (EntityRepository $repository): QueryBuilder {
            $parameter = $this->parameterFactory->createNamedParameter('isRecipient');

            return $repository
                ->createQueryBuilder($entityAlias = $this->createAlias())
                ->where($entityAlias.'.isRecipient = '.$parameter)
    ...
    {
        return $this->aliasFactory->createAlias(static::class, 'recipient');
    }
}
